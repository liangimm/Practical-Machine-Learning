...
```
So there are 19622 observations in the data set. Every observation contains 160 variables.

#Data clean and preprocessing
##Removing NA and "" columns
160 variables are too big for compter processing, and we need do the data clean and preprocesing firstly.
...
```
Now, there are only 53 columns left.

#Removing non-important columns
With above data processing, we succeed reducing the data set variable number from 160 to 53. But for the computer memory limitions, some r classification methods such as Random Forest classification etc are still dificult to perform. So I have to further reduce the data variables number.
...
We can see that there are only 39 variables left.
...
We can see that althogh we have trimed the variables number to 39, the Random Forest still has the above 0.9957 accuracy rate.

KNN's accuracy is about 92%, which is : 0.918.

So we get the Random Forest method's out of sample error rate as: 0.0043.

The KNN method's out of sample error rate as: 0.082.
...
#Conclusion
In this project, a predicting model manner of people do exercise was made. By reomving the almost NAs and balank columns, alomst non-changed columnes, just log info columns and the not important columns, the variables number was succeeded reduced from 160 to 39.

Wtih the random forest classification methode, the predication model's out of sample error rate was get at: 0.0043.
